...
if [ "$(hostname)" != "swarm-manager" ]; then
    echo " Ce script est prévu pour swarm-manager (hostname actuel : $(hostname))."
...
if [ "$SWARM_STATE" = "active" ]; then
    echo "  Un Swarm est déjà initialisé sur ce nœud."
    docker node ls
    echo ""
    echo "  Pour récupérer le token worker : docker swarm join-token worker"
    exit 0
fi
